document.addEventListener("DOMContentLoaded", () => {
  // Add each published Instagram Reel URL once here.
  // The same URL is automatically applied to the matching Reel buttons
  // in both the All and Videos views.
  ...
  document.querySelectorAll(".instagram-reel-link[data-reel]").forEach(link => {
    const reelNumber = link.dataset.reel;
    const reelUrl = instagramReelLinks[reelNumber]?.trim();

    if (reelUrl) {
      link.href = reelUrl;
      link.classList.remove("is-placeholder");
    } else {
      link.href = "https://www.instagram.com/sanjanacreates_18/";
      link.classList.add("is-placeholder");
    }
  });

  // Mobile navigation — robust hamburger + responsive resize handling
